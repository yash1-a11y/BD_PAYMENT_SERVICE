...
export function EditAdminModal({ admin, onClose, onUpdated }: Props) {
  const { token } = useAuth();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setIsSaving(true);
    try {
      const updated = await updateAdmin(token, admin.id, { email });
      onUpdated(updated);
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-40 px-4">
      <form onSubmit={handleSubmit} className="bg-card rounded-lg max-w-md w-full p-6">
        <h2 className="text-lg font-semibold text-navy mb-4">Edit admin</h2>
